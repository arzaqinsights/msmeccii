...
@section('title', 'Funding Initiative for MSMEs (2026) - MSMECCII')
        ...
        <div class="container space-y-32">
            ...
            <div class="grid md:grid-cols-3 gap-8">
                ...
                <div class="group relative bg-white p-10 rounded-lg border border-slate-200 hover:border-brand-primary transition-all duration-500 shadow-sm hover:shadow-xl">
                    <div class="w-16 h-16 bg-slate-50 rounded-lg flex items-center justify-center text-brand-primary mb-8 group-hover:bg-brand-primary group-hover:text-white transition-all border border-slate-100">
                        <i class="fa-solid fa-{{ $step['icon'] }} text-2xl"></i>
                    </div>
                    <h4 class="text-2xl font-black text-slate-900 mb-4 uppercase italic">{{ $step['t'] }}</h4>
                ...
                <div class="relative z-10 flex flex-col lg:flex-row items-center gap-16">
                    ...
                    <div class="lg:w-1/2 grid grid-cols-2 gap-6">
                        ...
                        <div class="p-6 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition-all text-center lg:text-left">
                            <i class="fa-solid fa-{{ $dev['icon'] }} text-2xl mb-4 text-brand-accent"></i>
                            <h4 class="text-xs font-black uppercase text-white mb-1">{{ $dev['title'] }}</h4>
                            <p class="text-[10px] text-white/50 uppercase tracking-widest">{{ $dev['desc'] }}</p>
                        </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="grid lg:grid-cols-2 gap-20 items-center">
                ...
                <div class="order-1 lg:order-2 space-y-8">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach(['Pre-screening', 'Lender Connect', 'Funding Camps', 'Mentorship'] as $tag)
                            <span class="px-4 py-2 bg-slate-900 text-white rounded-lg text-[10px] font-black uppercase tracking-widest">{{ $tag }}</span>
                        @endforeach
                    </div>
                </div>
            </div>
            ...
            <div class="text-center py-20 border-y border-slate-100">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center gap-6">
                    <a href="{{ route('contact') }}" class="w-full sm:w-auto px-12 py-6 bg-brand-primary text-white font-black uppercase tracking-widest rounded-lg hover:bg-slate-900 transition-all shadow-xl">
                        Apply for Funding
                    </a>
                    <a href="{{ route('register') }}" class="w-full sm:w-auto px-12 py-6 bg-white border-2 border-brand-primary text-brand-primary font-black uppercase tracking-widest rounded-lg hover:bg-brand-primary hover:text-white transition-all">
                        Join Membership
                    </a>
                </div>
            </div>
