    ...
    <section class="relative pt-32 pb-20 bg-slate-900 border-b border-brand-primary/20">
        @if($form->thumbnail)
            <div class="absolute inset-0">
                <img src="{{ asset($form->thumbnail) }}" alt="{{ $form->name }}"
            ...
            <form action="{{ route('join.forms.store', $form->slug) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="pb-8 border-b border-slate-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2">Full Name <span
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2">Email Address <span
                                    class="text-red-500">*</span></label>
                            <input type="email" name="email" placeholder="e.g. [EMAIL_ADDRESS]"
                                value="{{ old('email', auth()->check() ? auth()->user()->email : '') }}" required
                                class="w-full border border-slate-200 rounded-lg p-3 focus:border-brand-primary outline-none text-slate-900 font-bold placeholder:text-slate-300 placeholder:font-medium">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2">Phone Number <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2">Company Name <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="company_name" placeholder="e.g. ABC Pvt. Ltd."
                                value="{{ old('company_name', auth()->check() ? auth()->user()->company_name : '') }}"
                                required
                                class="w-full border border-slate-200 rounded-lg p-3 focus:border-brand-primary outline-none text-slate-900 font-bold placeholder:text-slate-300 placeholder:font-medium">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2">Designation <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="designation" placeholder="e.g. CEO"
                        ...
                        <div>
                            ...
                            <input type="text" name="website" placeholder="e.g. www.example.com"
                    ...
                    <div class="mt-6">
                        <label class="block text-xs font-bold text-slate-500 mb-2">Full Office/Company Address <span
                                class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2">City <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2">Pin Code <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="pincode" placeholder="e.g. 700001"
                                value="{{ old('pincode', auth()->check() ? auth()->user()->pincode : '') }}" required
                                class="w-full border border-slate-200 rounded-lg p-3 focus:border-brand-primary outline-none text-slate-900 font-bold placeholder:text-slate-300 placeholder:font-medium">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2">Country <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div x-show="field.type !== 'hidden_price' && isFieldVisible(field)" x-transition.opacity
                            ...
                            <template x-if="field.type === 'textarea'">
                                <textarea :name="'dynamic_fields[' + field.field_identifier + ']'"
                                    x-model="formData[field.field_identifier]"
                                    @input="handleFieldChange(field.field_identifier)" :placeholder="field.placeholder"
                                    :required="field.is_required && isFieldVisible(field)" rows="4"
                            ...
                            <template x-if="field.type === 'number'">
                                <div class="space-y-4 bg-slate-50/50 p-4 rounded-lg border border-slate-100 shadow-sm">
                                    <div class="flex justify-between items-center mb-1">
                                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Select
                                            Value</span>
                                        <div
                                            class="bg-brand-primary text-white px-3 py-1 rounded-full text-sm font-black shadow-lg shadow-brand-primary/20">
                                            <span
                                                x-text="formData[field.field_identifier] || field.options.min || 0"></span>
                                        </div>
                                    </div>
                                    ...
                                    <div
                                        class="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
                ...
                <div x-show="totalCalculated > 0" x-transition
                    class="mt-10 bg-brand-primary rounded-lg p-6 text-white relative overflow-hidden">
                    <!-- BG Pattern -->
                    <div class="absolute inset-0 opacity-10"
                        style="background-image: radial-gradient(#fff 1px, transparent 1px); background-size: 20px 20px;">
                    </div>

                    <h4 class="text-xs font-black uppercase tracking-widest text-white/50 mb-4 relative z-10">Application
                        Ledger</h4>
                    ...
                    <div class="space-y-3 relative z-10 border-b border-white/10 pb-4 mb-4">
                        ...
                        <div
                            class="pt-2 mt-2 border-t border-white/5 flex justify-between items-center text-sm font-medium">
                ...
                <div x-show="isManualPayment" x-transition
                    class="mt-8 p-8 bg-slate-50 border-2 border-dashed border-slate-200 rounded-lg">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h4 class="text-sm font-black text-slate-900 uppercase tracking-widest">Manual Bank Transfer
                            </h4>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        <div class="space-y-1">
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Beneficiary / Account
                                Name</p>
                            <p class="text-sm font-black text-slate-900">
                                {{ $site['account_name'] ?? 'MSME Chamber of Commerce and Industry of India' }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Bank Name</p>
                            <p class="text-sm font-black text-slate-900">{{ $site['bank_name'] ?? 'Contact Administrator' }}
                            </p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Account Number</p>
                            <p class="text-sm font-black text-slate-900 font-mono">
                                {{ $site['account_number'] ?? 'Not Configured' }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest">IFSC Code
                                (Domestic/India)</p>
                            <p class="text-sm font-black text-slate-900 font-mono">{{ $site['ifsc_code'] ?? 'N/A' }}</p>
                        </div>
                        @if(isset($site['swift_code']) && $site['swift_code'])
                            <div class="space-y-1">
                                <p class="text-xs font-black text-slate-400 uppercase tracking-widest">SWIFT / BIC Code
                                    (International)</p>
                    ...
                    <div class="mt-6 p-4 bg-amber-50 rounded-lg border border-amber-100 flex gap-3">
                        ...
                        <p class="text-[10px] font-bold text-amber-800 leading-relaxed">
                            After submission, please share the transaction receipt with our team at {{ $site['email_1'] ??
                            'support@msmeccii.com' }} or WhatsApp at +91 9810690843 for verification.
                        </p>
                    </div>
                </div>
                ...
                <div class="mt-8 pt-6 border-t border-slate-200">
                    <div
                        ...
                        <div class="flex items-center gap-3 text-slate-700">
                            ...
                            <div class="text-xs">
                                <span class="font-black block text-slate-900 uppercase tracking-widest">Secure
                                    Application</span>
                        ...
                        <div class="flex items-center gap-3 text-slate-700">
                            ...
                            <div class="text-xs">
                                <span class="font-black block text-slate-900 uppercase tracking-widest">Verified
                                    Portal</span>
                ...
                <div class="mt-10 pt-8 border-t border-slate-100">
                    ...
                    <button type="submit" :disabled="loading"
                        class="w-full bg-brand-primary hover:bg-brand-primary-dark text-white font-black py-4 rounded-lg transition-all shadow-lg text-lg drop-shadow-sm flex justify-center items-center gap-3 disabled:opacity-50 disabled:cursor-wait">
                        <template x-if="loading">
                            <i class="fa-solid fa-circle-notch fa-spin"></i>
                        </template>
                        <span
                            x-text="loading ? 'Processing...' : (isManualPayment ? 'Submit Application' : '{{ $form->submit_button_text }}')"></span>
                        <i x-show="!loading" class="fa-solid fa-arrow-right-long text-sm"></i>
                    </button>

                    <!-- WhatsApp CTA -->
                    <div class="mt-6 text-center">
                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Prefer to Talk
                            First?</p>
