...
<div class="px-8 py-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="<?php echo e(route('admin.articles.create')); ?>" class="px-5 py-2.5 bg-brand-primary text-white text-sm font-bold rounded-xl shadow-md hover:bg-brand-dark transition-all flex items-center gap-2">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="p-4 pl-6 flex items-center gap-4">
                                <div class="w-14 h-14 rounded-lg bg-slate-100 flex-shrink-0 overflow-hidden shadow-inner">
                                    <img src="<?php echo e($article->thumbnail_url); ?>" alt="Thumb" class="w-full h-full object-cover">
                                </div>
                                <div>
                                    <div class="font-bold text-slate-800 text-sm mb-0.5 truncate max-w-xs"><?php echo e($article->title); ?></div>
                                    <div class="text-xs text-slate-500 font-medium truncate max-w-xs"><?php echo e(url('/blog/' . $article->slug)); ?></div>
                                </div>
                            </td>
                            <td class="p-4">
                                <div class="flex flex-wrap gap-1">
                                    <?php if($article->category): ?>
                                        <?php $__currentLoopData = array_map('trim', explode(',', $article->category)); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <span class="bg-slate-100 text-slate-600 text-[11px] font-bold px-2 py-1 rounded-md uppercase tracking-wider"><?php echo e($cat); ?></span>
                            ...
                            <td class="p-4 text-xs font-bold text-slate-500">
                                <?php echo e($article->published_at ? $article->published_at->format('M d, Y') : 'N/A'); ?>

                            </td>
                            <td class="p-4 pr-6 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="<?php echo e(route('blog.show', $article->slug)); ?>" target="_blank" class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-100 transition-colors" title="View Live">
                                        <i class="fa-solid fa-eye text-xs"></i>
                                    </a>
                                    <a href="<?php echo e(route('admin.articles.edit', $article)); ?>" class="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center hover:bg-slate-200 transition-colors" title="Edit">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\msmeccii\resources\views/admin/articles/index.blade.php ENDPATH**/ ?>