...
<?php $__env->startSection('header', 'Media Gallery'); ?>
...
<div class="space-y-6">
         ...
         x-data="{
            ...
            async startUpload(e) {
                ...
                for (let i = 0; i < files.length; i++) {
                    ...
                    try {
                        const response = await fetch('<?php echo e(route('admin.gallery.store')); ?>', {
                            method: 'POST',
                            body: formData,
        ...
        <div x-show="showOrderModal" x-transition.opacity class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div @click.away="showOrderModal = false" class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden" x-data="galleryOrdering()">
                ...
                <div class="p-6">
                    ...
                    <form action="<?php echo e(route('admin.gallery.save-order')); ?>" method="POST">
                        <?php echo csrf_field(); ?>
    ...
    <script>
        function galleryOrdering() {
            return {
                items: <?php echo json_encode($categories->pluck('category')->toArray(), 15, 512) ?>,
                moveUp(index) {
                    if (index > 0) {
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <div x-data="{ editing: false, newName: '<?php echo e($category->category); ?>' }" class="group block relative rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all border border-slate-200 bg-white">
            <a href="<?php echo e(route('admin.gallery.show', base64_encode($category->category))); ?>">
                <div class="aspect-video w-full relative overflow-hidden bg-slate-100">
                    <img src="<?php echo e(asset($category->cover)); ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="<?php echo e($category->category); ?>">
            ...
            <div x-show="editing" x-transition.opacity class="absolute inset-0 z-20 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-4">
                <form action="<?php echo e(route('admin.gallery.update-category')); ?>" method="POST" class="w-full">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="old_category" value="<?php echo e($category->category); ?>">
            ...
            <div class="absolute bottom-0 left-0 w-full p-4 pointer-events-none">
                <div class="flex justify-between items-end">
                    <div class="flex-1 min-w-0">
                        <h4 class="text-white font-bold text-lg truncate drop-shadow-md"><?php echo e($category->category); ?></h4>
                        <div class="flex items-center gap-2 text-indigo-200 text-xs font-semibold mt-1">
                            <i class="fa-regular fa-image"></i> <?php echo e($category->image_count); ?> <?php echo e(Str::plural('Image', $category->image_count)); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\msmeccii\resources\views/admin/gallery/index.blade.php ENDPATH**/ ?>